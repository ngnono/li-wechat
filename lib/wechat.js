 ...
 * @return {Boolean}
 */
Wechat.prototype.checkSignature = function (req, token) {
    ...
    var arr = [token || this.token, timestamp, once];
    arr.sort();
 ...
 */
Wechat.prototype._process = function (req, res) {
    ...
    req.on('end', function () {
        ...
        xml2js.parseString(rawMessage, function (err, result) {
            ...
            if (self.listeners(eventName).length === 0) {
                ...
                res.send(200, 'unsupport ' + eventName);
            }
        });
    });
};

Wechat.prototype.process = function (req, res, token) {

    if (!this.checkSignature(req, token)) {
        res.send(400, 'incorrt sign');
        return;
    }
